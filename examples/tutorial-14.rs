        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                if let Some(tutorial) = self.tutorial.as_mut() {
                    ...
                    let t = (std::time::Instant::now() - tutorial.start).as_secs_f32() * 2.0;
                    let ang = t.sin();
                    ...
                    target.draw(&tutorial.shape, glium::index::NoIndices(glium::index::PrimitiveType::TriangleStrip), &tutorial.program,
                                &uniform! { model: model, view: view, perspective: perspective,
                                            u_light: light, diffuse_tex: &tutorial.diffuse_texture, normal_tex: &tutorial.normal_map },
                                &params).unwrap();
