...
impl<T: SurfaceTypeTrait + ResizeableSurface> Display<T> {
    ...
    pub fn rebuild(
        ...
        let context = {
            ...
            let ContextSurfacePair { context, surface } = borrow.as_ref().expect("display is valid");
            ...
            unsafe {
                config
                    ...
                    .make_current(new_surface.as_ref().unwrap_or(surface))
                    .map_err(|e| { DisplayCreationError::GlutinError(e) })?
            }
        };

        let surface = new_surface.unwrap_or_else(|| {
            self.gl_context.take().expect("the GL context will be replaced").surface
        });
